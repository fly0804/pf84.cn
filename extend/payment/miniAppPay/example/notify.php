...
class PayNotifyCallBack extends WxPayNotify
{
	...
	public function Queryorder($transaction_id)
	{
        ...
        trace('Queryorder:'.json_encode($input),'debug');
	...
	{
        trace('1NotifyProcess:'.json_decode($data),'debug');
        ...
        if (!$this->Queryorder($data["transaction_id"])) {
            $msg = "订单查询失败";
            //Log::DEBUG("call back:" . $msg);
            trace('2NotifyProcess:'.json_decode($data),'debug');
            return false;
        }
        ...
        if (stripos($order_sn, 'recharge') !== false) {
            ...
            if ((string)($orderInfo['amount'] * 100) != (string)$data['total_fee']) {
                trace('3NotifyProcess:'.json_decode($orderInfo),'debug');
                return false; //验证失败
            }
            ...
            if ((string)($orderInfo['order_amount'] * 100) != (string)$data['total_fee']) {
                trace('5NotifyProcess:','debug');
                // Log::DEBUG("call back:金额验证失败.");
                return false; //验证失败
            }
            //Log::DEBUG("call back:开始更新订单状态.");
            $asd = $OrderModel->updatePay(array('order_id'=>$orderInfo['order_id'],'money_paid'=>$orderInfo['order_amount'],'transaction_id'=>$data["transaction_id"]),'微信支付成功，流水号：'.$data["transaction_id"]);// 修改订单支付状态
            trace('updatePay_pay:'.$asd,'debug');
        }
