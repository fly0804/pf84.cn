...
class AdminController extends BaseController
{
	...
	public function ajaxEdit()
	{
		...
		if ($toggle) {
			if($field == 'isprize'){
				$data[$field] = $toggle === 'true' || $toggle === 1 ? 2 : 1;
			}else{
	...
	{
	    //创建数据表
	    $sql = "ALTER TABLE `".$table."` ADD `".$field."` double(8,2) NULL DEFAULT '0' ;";
	    Db::execute($sql);
	}

	public function intsertTable($table,$field,$data)
	{
		//清空表
		//Db::execute("truncate table ".$table);
	    //创建数据表
	    // $sql = "INSERT INTO ".$table."(".$flag.",mid) VALUES " ;
	    // foreach ($data as $key => $value) {
	    // 	$sql .= "('".$value['data']."', '".$value['mid']."'),";
	    // }
	    // $sql = rtrim($sql,',');
